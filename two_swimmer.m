...
% Kb = ones(Nb,1)/100;
% Kb(1:10) = 1;
ZX1 = Lx/2; % fixed point; first point of the filament
ZY1 = 2*Ly/3; % Y
ZX2 = Lx/2;
ZY2 = Ly/3;
...
if movie_or_not == 1
    video = VideoWriter('twoswimmer_May6t0.mp4','MPEG-4');
    video.FrameRate = 30;
    open(video);
end
...
Y2 = X2; % Massive boundary
Z2 = [ZX2 ZY2]; % Fix the first point;
V2 = zeros(Nb,2);
...
for clock=1:clockmax
  Z1(1) = ZX1 + Af*sin(2*pi*clock*dt/T);
  Z2(1) = ZX2 + Af*sin(2*pi*clock*dt/T);
  THETA = -thetaf*cos(2*pi*clock*dt/T) + alpha;
  XX1 = X1 + (dt/2)*interp(u,X1);
  XX2 = X2 + (dt/2)*interp(u,X2);
  YY1 = Y1 + (dt/2)*V1;
  YY2 = Y2 + (dt/2)*V2;
  FF1 = ForceMagSwimmer(XX1,YY1,Z1,THETA);
  FF2 = ForceMagSwimmer(XX2,YY2,Z2,THETA);
  ...
  if mod(clock,1000)==0
      ...
      plot(ZX1, ZY1,'ro')
      plot(ZX2, ZY2,'ro')
      plot(mod(X1(1,1),Lx),mod(X1(1,2),Ly),'rx')
      plot(mod(X2(1,1),Lx),mod(X2(1,2),Ly),'rx')
      axis([0,Lx,0,Ly])
%       caxis(valminmax)
      axis equal
      axis manual
